...
function calcularTalla(texto) {
  ...
  if (alto >= 58 && alto <= 64) return 'M'
  if (alto >= 67 && alto <= 70) return 'L'
  if (alto >= 71 && alto <= 75) return 'XL'
  if (alto < 55) return 'S'
  if (alto < 66) return 'M'
  if (alto < 71) return 'L'
  return 'XL'
...
async function handleMessage(jid, texto, hasMedia) {
  ...
  if (t === '7' || t === 'asesor' || t.includes('asesor') || t.includes('humano')) {
    ...
    for (const num of owners) {
      await sendMessage(`${num.trim()}@s.whatsapp.net`, `👤 *Cliente solicita asesor*\nNúmero: ${jid.replace('@s.whatsapp.net', '')}`)
    }
    return
  }
  ...
  if (session.state === STATES.MENU) {
    if (t === '1' || t.includes('medida') || t.includes('talla')) {
      ...
      await sendImage(jid, process.env.IMG_MEDIDAS_URL, '📏 *Guía de medidas — Forros BlockBag*\n\nMide tu maleta *sin contar las ruedas* y envíame:\n\n↕️ *Alto* en cm\n↔️ *Ancho* en cm\n\nEjemplo: _65 45_')
      return
    }
  ...
  if (session.state === STATES.ESPERANDO_MEDIDAS) {
    ...
    const tallaMsg = talla ? `✅ Con esas medidas tu talla recomendada es *${talla}*.\n\n` : `✅ Medidas registradas: ${texto}\n\n`
  ...
  if (session.state === STATES.ESPERANDO_DISENO) {
    ...
    await sendMessage(jid, `✅ *Diseño registrado:* ${texto}\n\n📝 Para finalizar tu solicitud envíanos:\n\n👤 *Nombre completo*\n🏠 *Dirección de entrega*\n🏙️ *Ciudad*\n📱 *Teléfono de contacto*\n\nEnvíalos en un solo mensaje 👇`)
    return
  }
  ...
  if (session.state === STATES.OFERTA_CANDADO) {
    ...
    if (quiere || noQuiere) {
      ...
      const msg = quiere ? `✅ *Candado agregado* 🔒 +$${Number(process.env.CANDADO_PRECIO || 10000).toLocaleString('es-CO')}\n\n` : 'De acuerdo, sin candado.\n\n'
  ...
  if (session.state === STATES.ESPERANDO_DATOS_ENVIO) {
    ...
    const owners = (process.env.OWNER_NUMBERS || '').split(',').filter(Boolean)
    for (const num of owners) {
      await sendMessage(`${num.trim()}@s.whatsapp.net`, `📦 *Pedido contra entrega*\nCliente: ${jid.replace('@s.whatsapp.net', '')}\nDatos: ${texto}`)
    }
    return
  }
...
app.post('/webhook/orders/create', verificarShopify, async (req, res) => {
  res.sendStatus(200)
  try { const order = req.body; const phone = limpiarTelefono(order.billing_address?.phone || order.phone); if (!phone) return; await sendMessage(phone + '@s.whatsapp.net', msgPedidoNuevo(order)) } catch (e) { console.error('Error:', e.message) }
})
app.post('/webhook/orders/paid', verificarShopify, async (req, res) => {
  res.sendStatus(200)
  try { const order = req.body; const phone = limpiarTelefono(order.billing_address?.phone || order.phone); if (!phone) return; await sendMessage(phone + '@s.whatsapp.net', msgPagoConfirmado(order)) } catch (e) { console.error('Error:', e.message) }
})
app.post('/webhook/orders/fulfilled', verificarShopify, async (req, res) => {
  res.sendStatus(200)
  try { const order = req.body; const phone = limpiarTelefono(order.billing_address?.phone || order.phone); if (!phone) return; const tracking = order.fulfillments?.[0] || {}; await sendMessage(phone + '@s.whatsapp.net', msgEnvioDespachado(order, { number: tracking.tracking_number, company: tracking.tracking_company, url: tracking.tracking_url })) } catch (e) { console.error('Error:', e.message) }
})
app.post('/webhook/checkout/abandoned', verificarShopify, async (req, res) => {
  res.sendStatus(200)
  try { const checkout = req.body; const phone = limpiarTelefono(checkout.billing_address?.phone || checkout.phone); if (!phone) return; await sendMessage(phone + '@s.whatsapp.net', msgCarritoAbandonado(checkout, 1)) } catch (e) { console.error('Error:', e.message) }
})
app.post('/api/send', verificarApi, async (req, res) => {
  try { const { phone, message } = req.body; if (!phone || !message) return res.status(400).json({ error: 'Falta phone o message' }); await sendMessage(limpiarTelefono(phone) + '@s.whatsapp.net', message); res.json({ ok: true }) } catch (e) { res.status(500).json({ error: e.message }) }
})
app.post('/api/reporte', verificarApi, async (req, res) => {
  try { const statsBase = await getStatsHoy(); const stats = { ...statsBase, ...req.body }; const numeros = (process.env.OWNER_NUMBERS || '').split(',').filter(Boolean); for (const num of numeros) { await sendMessage(num.trim() + '@s.whatsapp.net', msgReporteDiario(stats)) }; res.json({ ok: true }) } catch (e) { res.status(500).json({ error: e.message }) }
})
...
cron.schedule(`${minuto} ${Number(hora) + 5} * * *`, async () => {
  try { const stats = await getStatsHoy(); const numeros = (process.env.OWNER_NUMBERS || '').split(',').filter(Boolean); for (const num of numeros) await sendMessage(num.trim() + '@s.whatsapp.net', msgReporteDiario(stats)) } catch (e) { console.error('Error reporte:', e.message) }
}, { timezone: 'UTC' })
